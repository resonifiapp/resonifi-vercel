    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <Card className="bg-slate-900/60 border-slate-800">
        ...
        <CardContent className="space-y-6">
          {step === 1 && (
            <>
              <Slider title={`Mood: ${mood}/10`} minLabel="Low" maxLabel="Great"
                value={mood} setValue={setMood} min={0} max={10} step={1} />
              <Slider title={`Energy: ${energy}/10`} minLabel="Exhausted" maxLabel="Energized"
                value={energy} setValue={setEnergy} min={0} max={10} step={1} />
            </>
          )}

          {step === 2 && (
            <>
              <Slider title={`Sleep (hours): ${sleepHrs}/12`} minLabel="0h" maxLabel="12h"
                value={sleepHrs} setValue={(v)=>setSleepHrs(Number(v))} min={0} max={12} step={0.5} />

              <div>
                <div className="mb-2 text-sm text-slate-300">Exercise (minutes)</div>
                <div className="flex items-center gap-3">
                  <Button variant="outline" onClick={()=>setExerciseMin(m=>Math.max(0,m-5))}><Minus className="h-4 w-4"/></Button>
                  ...
                  <Button variant="outline" onClick={()=>setExerciseMin(m=>Math.min(180,m+5))}><Plus className="h-4 w-4"/></Button>
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-8 gap-2">
                  {[0,0.5,1,1.5,2,2.5,3,4].map((lit)=>(
                    <button key={lit} onClick={()=>setWaterL(lit)}
                      className={`rounded-lg border px-3 py-2 text-sm ${
                        waterL===lit ? "border-teal-400 bg-teal-500/10 text-teal-300"
          ...
          {step === 3 && (
            <>
              <Field label="Gratitude" placeholder="A small moment you appreciated today."
                value={gratitude} onChange={setGratitude}/>
              <Field label="Act of kindness" placeholder="Something you did (or received) that brought ease."
                value={kindness} onChange={setKindness}/>
            </>
          )}

          {step === 4 && (
            <div className="space-y-4">
              <Row label="Mood" value={`${mood}/10`} />
              <Row label="Energy" value={`${energy}/10`} />
              <Row label="Sleep" value={`${sleepHrs}h / 12h`} />
              <Row label="Exercise" value={`${exerciseMin} min / 60`} />
              <Row label="Water" value={`${waterL}L (target 3L)`} />
              {gratitude && <Row label="Gratitude" value={gratitude} />}
              {kindness && <Row label="Kindness" value={kindness} />}
            </div>
          )}
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            {step<4 ? (
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" onClick={()=>saveSnapshot(false)}>Save locally</Button>
                <Button onClick={()=>{ saveSnapshot(true); window.location.hash="#/home"; }}>
...
function Slider({ title, minLabel, maxLabel, value, setValue, min, max, step }) {
  return (
    <div className="rounded-xl border border-slate-700 bg-slate-900/70 p-4">
      <div className="text-sm font-medium text-slate-200">{title}</div>
      <div className="mt-1 flex justify-between text-xs text-slate-400"><span>{minLabel}</span><span>{maxLabel}</span></div>
